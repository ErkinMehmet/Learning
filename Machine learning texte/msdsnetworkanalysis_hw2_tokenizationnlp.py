# -*- coding: utf-8 -*-
"""MSDSNetworkAnalysis_HW2_TokenizationNLP.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1SgOw_SkaXvpDrfBpWvZbXQXBVHFlqpJh
...
def unique_tokens(tweets, tokenizer=nltk.TweetTokenizer().tokenize):
    ...
    for twi in tweets:
      twTxt=None
      if isinstance(twi, bytes):
         tw=json.loads(twi.decode('utf-8'))
      else:
        tw=twi
      ...
      if not isinstance(tw,str):
        if 'text' in tw:
          if not tw['text']==None:
            twTxt=tw['text']
        if 'full_text' in tw:
          if not tw['full_text']==None:
            twTxt=tw['full_text']
      else:
        if isinstance(tw, str):  # Check if `tw` is a string
          twTxt=tw.encode('utf-8')
...
def token_counts(tweets,
    ...
    for tw in tweets:
      ...
      if not twTxt==None:
        tks=tokenizer(twTxt)
        tagged=tagger.tag(tks)
        if parts_of_speech:
          filtered=[w for w,t in tagged if t in parts_of_speech]
        else:
          filtered=[w for w,t in tagged]
        for f in filtered:
          if f in counts:
            counts[f]+=1
          else:
